...
def get_pixel_points_ellipse(img_width, img_height, init_width, init_height,
                             offset, is_filled, bitwise_not = False):
    ...
    points = np.fliplr(np.reshape(pixel_points,
                                           [len(pixel_points), 2]))
    return points
...
class ActiveContourFTC:
    ...
    def run_active_contour(self):
        if self.hasEllipse:
            ...
            # L_in points
            self.L_in = \
                get_pixel_points_ellipse(self.img_width,
                                         self.img_height, self.init_width,
                                         self.init_height, 0, is_filled=False)
            ...
            interior_points = \
                get_pixel_points_ellipse(self.img_width,
                                         self.img_height, self.init_width,
                                         self.init_height, 0, True)
            interior_points = find_common_array(interior_points, self.L_in)
            ...
            self.L_out = \
                get_pixel_points_ellipse(self.img_width,
                                         self.img_height, self.init_width,
                                         self.init_height, 1, is_filled=False)
            ...
            exterior_points = \
                get_pixel_points_ellipse(self.img_width,
                                         ...
                                         bitwise_not = True)
            exterior_points = find_common_array(exterior_points, self.L_out)
            ...
            interior_points = \
                get_pixel_points_rectangle(self.img_width,
                                           self.img_height, self.init_width,
                                           self.init_height, 0, is_filled=True)
            interior_points = find_common_array(interior_points, self.L_in)
            self.phi[interior_points[:, 0], interior_points[:, 1]] = -3
            ...
            exterior_points = \
                get_pixel_points_rectangle(self.img_width,
                                           self.img_height, self.init_width,
                                           self.init_height, -1,
                                           is_filled=True, bitwise_not = True)
            exterior_points = find_common_array(exterior_points, self.L_out)
            self.phi[exterior_points[:, 0], exterior_points[:, 1]] = 3
    ...
    def find_redundant_points_Lout(self, points):
        is_redundant = np.zeros(len(points),dtype=bool)
    ...
    def compute_internal_speed_Fint(self, point):
        f_int = 0
        x, y = point
    ...
    def do_one_iteration_in_cycle1(self):
        ...
        speeds = self.compute_external_speed_Fd(self.L_out[0], self.L_out[1])
        if speeds.any() > 0:
            in_points = self.L_out[speeds > 0]
            self.do_specific_in_step(in_points[0], in_points[1])
            self.switch_in(in_points)
            if not self.hasOutwardEvolution:
                self.hasOutwardEvolution = True
    ...
    def do_one_iteration_in_cycle2(self):
        for point in self.L_out:
            if self.Ns == self.Ns_max - 1:
                self.lists_length += 1
            if self.compute_internal_speed_Fint(point) < 0:
                self.do_specific_in_step(point[0], point[1])
        if self.hasCleaning:
            self.eliminate_redundant_in_Lin()
    ...
    def switch_in_vector(self, points):
        ...
        np.vstack((self.L_in, points))
        self.phi[x, y] = -1
        self.add_neighbors_to_Lout(x, y)

    def add_neighbors_to_Lout(self,x,y):
        x1 = x - 1
        y1 = y - 1
        x2 = x + 1
        y2 = y + 1
